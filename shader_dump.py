...
def handle_shader(shader, out_path):
	...
	utils.make_dirs(fname)

	if "m_SubProgramBlob" in shader._obj:
		compressed = unitypack.utils.BinaryReader(BytesIO(shader._obj["m_SubProgramBlob"]))
		# decompress lz4 frame
		lz4 = unitypack.utils.lz4_decompress(
			shader._obj["m_SubProgramBlob"],
			shader._obj["decompressedSize"]
		)
		uncompressed = unitypack.utils.BinaryReader(BytesIO(lz4))
		...
		for i, (o, l) in enumerate(subshaders):
			...
			utils.write_to_file(fname + str(i) + ".sub", data, "wb")

		utils.write_to_file(fname + ".bin", lz4, "wb")
		utils.write_to_file(fname + ".shaderlab", shader.script, "w")
